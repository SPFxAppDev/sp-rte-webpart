...
export class RichText extends React.Component<IRichTextProps, IRichTextState> {
  ...
  private ddStyleOpts = [{
    key: 0,
    text: strings.CUSTOM_HeaderNormalText,
    data: {}
  }, {
    key: 2,
    text: strings.CUSTOM_HeaderH2,
    data:
      { className: styles.toolbarButtonH2 }
  }, {
    key: 3,
    text: strings.CUSTOM_HeaderH3,
    data:
      { className: styles.toolbarButtonH3 }
  }, {
    key: 4,
    text: strings.CUSTOM_HeaderH4,
    data:
      { className: styles.toolbarButtonH4 }
  }, {
    key: 7,
    text: strings.CUSTOM_HeaderBlockQuote,
    data:
  ...
  private ddAlignOpts = [{
    key: 'left',
    text: strings.CUSTOM_AlignLeft,
    data: { icon: 'AlignLeft' }
  }, {
    key: 'center',
    text: strings.CUSTOM_AlignCenter,
    data: { icon: 'AlignCenter' }
  }, {
    key: 'right',
    text: strings.CUSTOM_AlignRight,
    data: { icon: 'AlignRight' }
  }];

  private ddListOpts = [{
    key: 'bullet',
    text: strings.CUSTOM_ListBullet,
    data: { icon: 'BulletedList' }
  }, {
    key: 'ordered',
    text: strings.CUSTOM_ListNumbered,
    ...
    return (
      <Dialog hidden={this.state.hideDialog}
        ...
        dialogContentProps={{
          ...
          title: strings.CUSTOM_InsertLinkTitle,
        }}
        modalProps={{
        ...
        <TextField label={strings.CUSTOM_AddressFieldLabel}
        ...
        <TextField label={strings.CUSTOM_TextToDisplayLabel}
          ...
          <div className={`ms-Dialog-actionsRight ${styles.actionsRight}`}>
            {
              this.state.selectedUrl && (
                <Link className={`${styles.action} ${styles.unlinkButton}`} onClick={this.handleRemoveLink}>{strings.CUSTOM_RemoveLinkLabel}</Link>
              )
            }
            <PrimaryButton className={styles.action} onClick={this.handleCreateLink} text={strings.CUSTOM_SaveButtonLabel} disabled={this.checkLinkUrl()} />
            <DefaultButton className={styles.action} onClick={this.closeDialog} text={strings.CUSTOM_CancelButtonLabel} />
          </div>
        </DialogFooter>
      </Dialog>
    );
  ...
  * Renders the "Insert Image" dialog
  ...
  private renderImageDialog = (): JSX.Element => {
    return (
      <Dialog hidden={this.state.hideImageDialog}
        ...
        dialogContentProps={{
          type: DialogType.normal,
          title: strings.CUSTOM_InsertImageTitle,
        }}
        modalProps={{
          className: styles.insertLinkDialog,
          isBlocking: true,
        ...
        <DialogFooter className={styles.actions}>
          <div className={`ms-Dialog-actionsRight ${styles.actionsRight}`}>
            <PrimaryButton className={styles.action} onClick={this.handleInsertImage} text={strings.CUSTOM_SaveButtonLabel} disabled={this.checkImageLinkUrl()} />
            <DefaultButton className={styles.action} onClick={this.closeImageDialog} text={strings.CUSTOM_CancelButtonLabel} />
          </div>
        </DialogFooter>
      </Dialog>
    );
  ...
  public render(): React.ReactElement<IRichTextProps> {
    ...
    if (!isEditMode) {
      EventHandler.Fire("OnBeforeRTERenderInDisplayMode", this);
      return (
        <>
    ...
    {EventHandler.Fire("OnQuillRegister", Quill);}
    ...
    return (
      <div ref={(ref) => this._wrapperRef = ref} className={`${styles.richtext && this.state.editing ? 'ql-active' : ''} ${this.props.className}`}>
        <div id={this._toolbarId} style={{ top: this.state.wrapperTop }}>
          ...
          {
            showImage && (
              <TooltipHost content={strings.CUSTOM_ImageTitle}
          ...
          {EventHandler.Fire("OnRTEHeaderControlsRender", this)}
          {
            showMore && (
              <TooltipHost content={strings.CUSTOM_MoreTitle}
